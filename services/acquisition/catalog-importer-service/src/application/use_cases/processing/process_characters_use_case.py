...
class ProcessCharactersUseCase:
    ...
    async def execute(self):
        try:
            ...
            logger.error(
                f"Unexpected error during fetching unprocessed characters: {e}")
            return
        ...
        for unprocessed_character in unprocessed_characters:
            logger.info(
                f"Processing character {unprocessed_character.name} (ID: {unprocessed_character.id})")
            try:
                # ------------- Set gender id from name -----------------
                logger.info(
                    f"Setting gender id for character {unprocessed_character.name} (ID: {unprocessed_character.id})")
                await self._set_gender_id(character=unprocessed_character)

                logger.info(
                    f"Processing name for character {unprocessed_character.name} (ID: {unprocessed_character.id})")
                await self._process_name(unprocessed_character)

                # ------------- Save unprocessed character -----------------
                logger.info(
                    f"Saving character {unprocessed_character.name} (ID: {unprocessed_character.id})")
                character = await self.characters_repo.save_unprocessed_character(unprocessed_character)
                logger.info(
                    f"Character {unprocessed_character.name} saved (ID: {unprocessed_character.id})")
                try:

                    # ------------- Set image to processing -----------------
                    logger.info(
                        f"Setting image to process for character {unprocessed_character.id}")
                    await self._set_image_to_process(character)
                    logger.info(
                        f"Image set to process for character {unprocessed_character.id}")

                    # ------------- Set processing_state to processed-----------------
                    logger.info(
                        f"Settings parsed character {unprocessed_character.name} as processed")
                    await self.parsed_character_repo.set_character_as_processed(unprocessed_character.id)
                    logger.info(
                        f"Character {unprocessed_character.name} marked as processed")

                except Exception as e:
                    logger.error(
                        f"Error by processing {unprocessed_character.id}: {e}")
                    logger.error(
                        f"Removing saved character {unprocessed_character.id} due to image processing error")
