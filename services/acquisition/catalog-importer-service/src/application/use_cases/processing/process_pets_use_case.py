...
#
# from monstrino_core.exceptions import EntityNotFound, DBConnectionError, EntityAlreadyExists
#
# from monstrino_repositories.repositories import (
#     ImageReferenceOriginRepo, ParsedImagesRepo, ParsedPetRepo, PetRepo,
#     CharactersRepo
# )
...
#             logger.error(
#                 f"Unexpected error during fetching unprocessed pets: {e}")
#             return
#
...
#
#         logger.info(
#             f"============== Starting processing of {len(unprocessed_pets)} pets ==============")
#         for i, unprocessed_pet in enumerate(unprocessed_pets):
#             logger.info(
#                 f"Processing pet {unprocessed_pet.name} (ID: {unprocessed_pet.id})")
#
#             try:
...
#                 except Exception as e:
#                     logger.error(
#                         f"Error processing pet {unprocessed_pet.name} (ID: {unprocessed_pet.id}): {e}")
#                     await self.set_pet_as_processed_with_errors(unprocessed_pet)
#
#                     if pet:
#                         logger.error(
#                             f"Removing saved pet {pet.id} due some error")
#                         await self.pets_repo.remove_unprocessed_pet_by_id(pet.id)
#
#             except EntityAlreadyExists as e:
#                 logger.error(
#                     f"Pet {unprocessed_pet.name} (ID: {unprocessed_pet.id}) already exists: {e}")
#                 await self.set_pet_as_already_exists(unprocessed_pet)
#             except Exception as e:
#                 logger.error(
#                     f"Unexpected error during processing pet {unprocessed_pet.name} (ID: {unprocessed_pet.id}): {e}")
#                 await self.set_pet_as_processed_with_errors(unprocessed_pet)
#         return
#
#     async def _get_unprocessed_pets(self, count: int) -> list[ParsedPet] | None:
#         try:
#             return await self.parsed_pet_repo.get_unprocessed_pets(count)
...
#         await self.parsed_pet_repo.set_pet_as_already_exists(pet.id)
#
#     async def set_pet_as_processed(self, pet: ParsedPet):
#         logger.debug(f"Setting parsed pet {pet.name} as 'processed'")
#         await self.parsed_pet_repo.set_pet_as_processed(pet.id)
#
#     async def set_pet_as_processed_with_errors(self, pet: ParsedPet):
#         logger.error(
#             f"Setting parsed pet {pet.name} as 'processed_with_errors'")
#         await self.parsed_pet_repo.set_pet_as_processed_with_errors(pet.id)
