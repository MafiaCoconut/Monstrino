...
async def test_process_single_series_success(
    ...
    async with uow_factory.create() as uow:
        ...
        parent_resolver_mock.resolve.return_value = None

    uc = ProcessSingleSeriesUseCase(
        uow_factory=uow_factory,
        parent_resolver_svc=parent_resolver_mock,
    )
    ...
    async with uow_factory.create() as uow:
        async with uow:
            series = await uow.repos.series.get_one_by_fields_or_none(display_name=parsed_series.name)
            ...
            parsed_series_after: ParsedSeries = await uow.repos.parsed_series.get_one_by_fields_or_none(name=parsed_series.name, processing_state=ProcessingStates.PROCESSED)
