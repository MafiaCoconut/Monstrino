        ...
        async with async_session_factory() as session:
            try:
                ...
                raise SavingParsedRecordWithErrors(
                    F"Series with name {data.name} already exists")
            except Exception as e:
                raise SavingParsedRecordWithErrors(
                    f"Error saving series {data.name}: {e}") from e

    async def get_by_name(self, series_name: str) -> Optional[ParsedSeries]:
        async with async_session_factory() as session:
            try:
                query = select(ParsedSeriesORM).where(
                    ParsedSeriesORM.name == series_name)
                ...
                if not series_orm:
                    raise EntityNotFound(
                        f"Series with name {series_name} not found")
                ...
                raise SavingParsedRecordWithErrors(
                    f"Error getting series {series_name}: {e}") from e

    async def get_parent_series(self, series_name: str) -> Optional[ParsedSeries]:
        async with async_session_factory() as session:
            try:
                query = select(ParsedSeriesORM).where(and_(
                    ParsedSeriesORM.name == series_name, ParsedSeriesORM.series_type == 'series_prime'))
                result = await session.execute(query)
    ...
    async def set_parent_id(self, parsed_series: ParsedSeries) -> Optional[ParsedSeries]:
        async with async_session_factory() as session:
            try:
                ...
                if not series_orm:
                    raise EntityNotFound(
                        f"Series with name {parsed_series.name} not found")
                ...
                raise SavingParsedRecordWithErrors(
                    f"Error getting series {parsed_series.name}: {e}")
        ...
        async with async_session_factory() as session:
            try:
                ...
                raise DBConnectionError(
                    f"Failed to delete series {parsed_series.name}: {e}")
